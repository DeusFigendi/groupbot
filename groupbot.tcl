...
proc send_help { my_command } {	
	...
	set help_text "$help_text You can controll me by several commands which I'll be teaching you now."
	set help_text "$help_text First of all: Whenever you send anything to me, do it by writing a regular posting and **mention** me in it. I do not handle *'conversation'* (private messages) at the moment.\\nI have to be able to read the posting so you have to send it publicly or to an aspect I'm in. So maybe you wanna create an own aspect for me or for bots in general.\\n Any command send to me has to be in the first line of your posting (several other lines might be payload).\\n\\n"
	set help_text "$help_text\### Groupnames\\n\\n"
	set help_text "$help_text Whenever you refer to a groupname start with a \\$groupprefix\. So if you're talking about the group *'mygroup'* you have to use \\$groupprefix\mygroup.\\n\\n"
	...
	set help_text "$help_text Joining a group is nearly the same as creating one, just use **subscribe** or **enter** and the group's name.\\n\\n"
	...
	set help_text "$help_text If you are the last admin of a group and leave it, then the user having the longest membership/subscription inherits the admin status. If no user is left after you leave, the group will be disbanded.\\n\\n"
	set help_text "$help_text\### Setup prefferences\\n\\n"
	set help_text "$help_text The command-word to setup preferences of a group is **set** or **setup**. There are some subcommands to use. Of course you have to be admin of the group to do any setup.\\n\\n"
	set help_text "$help_text\* **invite_only** makes the group to a closed one, no one can subscribe to it without being invited. (not yet implemented)\\n"
	...
	set help_text "$help_text * **prohibit_extern_postings** will prevent non-subscribers from sending postings to the group. (default but not yet implemented)\\n"
	set help_text "$help_text\* **open_input** or **open_post(ing)s** or **allow_ext(ern(al))_post(ing)s** will allow non-subscribers to send postings to the group.\\n"
	set help_text "$help_text\* **public** or **always_public** makes any group-posting in the future public. No matter whether it's set as public or not. (more about public posts in the *'sending'* section)\\n"
	set help_text "$help_text\* **private** makes group-posting by default only visible to the group, so if you don't tell me to make a posting public, I will send it *limited* to an aspect the group is bound to.\\n"
	set help_text "$help_text\* **admin** \\\[diaspora handle\\\] makes another user an admin of the group. You will not lose your admin-status in that case. Write the diaspora-handle of the new admin directly behind the keyword **admin** (seperated by a space). The new admin is added to the group as an admin **and** as an subscriber if he isn't already.\\n"
	set help_text "$help_text\* **description** tells me to use the *payload* of the posting as the description of the group. (You can set a description and it is saved, but for now there is no way to display it.). In this case 'payload' means  the text starting one or two lines below the keyword **description**.\\n\\n"
	...
	set help_text "$help_text Sending a posting is simple and needs only one (or two) keywords. If none of the command-words appears in the first line I'll take the posting as... as a posting to send. Use the *'payload'* to write your actual posting.\\n\\n"
	...
	set help_text "$help_text The only keyword (except the groupname) is **public**. Usually a post you send to the group is *limited* to the group (using the aspect-functionality of diaspora). But if you use the keyword **public** in the first line (or the group is set to be always public) I will repost it as a *public* posting and *mention* all subscribers (so they'll get a notification).\\n\\n"
