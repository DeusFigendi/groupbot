...
proc send_help { my_command } {	
	...
	global groupprefix
	set help_text "## Help\\n\\n"
	set help_text "$help_text Hello and welcome, you aspected me and asked me for help. So I'll tell you how this stuff works.\\n\\n"
	set help_text "$help_text This bot is a group-bot and I offer the functionality like you might know from mailing-lists I'm a distributor for messages."
	set help_text "$help_text You can controll me by several commands I'll teach you now."
	set help_text "$help_text First to mention: Whenever you send anything to me do it by writing a regular posting and **mention** me in it. I do not take any *'conversation'* (private messages) or something at the moment.\\nBe sure I can read that posting so you have to send it public or to an aspect I'm in. So maybe you wanna create an own aspect for me or... for bots in general.\\n Any command send to me 'happens' in the first line of your posting (several other lines might be payload).\\n\\n"
	set help_text "$help_text\### Groupnames\\n\\n"
	set help_text "$help_text Whenever you reffer a groupname start with a \\$groupprefix\. So if you're talking about the group *'mygroup'* you'd use \\$groupprefix\mygroup.\\n\\n"
	set help_text "$help_text\### Founding a group\\n\\n"
	set help_text "$help_text The command-word to found a group is **create** or **found** and please tell me the name of the group ;D.\\nSo if you'd like to create the group *'mygroup'* write a posting like\\n\\n"
	...
	set help_text "$help_text Enterin a group is nearly like creating one, just use **subscribe** or **enter** and the groups name.\\n\\n"
	...
	#set help_text "$help_text If you were the last admin of a group and left it, the user who's as longest subscriber will become the new admin. If no user is left the group is droped.\\n\\n"
	set help_text "$help_text If you are the last admin of a group and leave it, then the active user with the longest membership/subscription inherits the admin status. If no user is left after you leave, the group will be disbanded.\\n\\n"
	set help_text "$help_text\### Setup prefferences\\n\\n"
	set help_text "$help_text The command-word to setup preferences of a group is **set** or **setup**. There are some subcommands to use. In any case you have to be admin of that group to do any setup.\\n\\n"
	...
	set help_text "$help_text\* **open_access** is the oposite, it makes the group to an open one, anyone can subscribe it without beeing invited. (default)\\n"
	set help_text "$help_text\* **closed_input** or\\n"
	set help_text "$help_text * **closed_posting** or\\n"
	set help_text "$help_text * **closed_posts** or\\n"
	set help_text "$help_text * **deny_external_postings** or\\n"
	set help_text "$help_text * **forbid_extern_posting** or\\n"
	set help_text "$help_text * **disallow_ext_posts** or\\n"
	set help_text "$help_text * **prohibit_extern_postings** will prevent non-subscribers sending postings into the group. (default but not implemented)\\n"
	set help_text "$help_text\* **open_input** or **open_post(ing)s** or **allow_ext(ern(al))_post(ing)s** will allow non-subscribers sending postings into the group.\\n"
	set help_text "$help_text\* **public** or **always_public** makes any group-posting in the future public. Whenever set as public or not. (more about public posts in the *'sending'* section)\\n"
	set help_text "$help_text\* **private** makes group-posting usual group-internal, so if you don't tell me to make a posting public I will send it *limited* to an aspect the group is bound to.\\n"
	set help_text "$help_text\* **admin** \\\[diaspora handle\\\] makes another user to an admin of the group. You will not lose your admin-status in that case. Write the diaspora-handle of the new admin directly behind the keyword **admin** (seperated by a space). The new admin is added to the group as an admin **and** subscriber if she isn't allready.\\n"
	set help_text "$help_text\* **description** tells me to use the *payload* of the posting as the description of the group. (You can setup a description and it is saved, but for now there is no implementation to output it.). 'Payload' in this case means you write the keyword **description** into the first line. Then press Return one or two times and write your description.\\n\\n"
	set help_text "$help_text You can setup multible stuff in one posting. **Example:**\\n\\n"
	...
	set help_text "$help_text I don't care about the order of the commands/keywords just be sure to write the diaspora-handle directly after the *'admin'* keyword, everything else... pffft (that includes the groups name, just be sure to prefix it with a \\$groupprefix ).\\n\\n"
	set help_text "$help_text\### Sending postings\\n\\n"
	set help_text "$help_text Sending a posting is simple and it has just one (or two) keywords. If no of the command-words apears in the first line I'll take the posting as... as posting.. to send. Use the *'payload'* to writer your actual posting.\\n\\n"
	...
	set help_text "$help_text That style. The only keyword (except the groupname) is **public**. Usually a post you send to the group is *limited* to the group (by the aspect-functionality of diaspora). But if you use the keyword **public** in the first line (or the group is set to be always public) I will repost it as a *public* posting and *mention* all subscribers (so the'll get a notification).\\n\\n"
